# Publishing AudioWinFix to WinGet and Chocolatey
...
| --- | --- | --- | --- |
| WinGet | `AudioWinFix-win-Setup.exe` | user (declared in the manifest) | the app itself |
| Chocolatey | `AudioWinFix-win-Portable.zip` | machine | `choco upgrade` |
...
```powershell
choco pack packaging/chocolatey/audiowinfix.nuspec --out packaging/chocolatey
choco install audiowinfix --source packaging/chocolatey --version <version> -y
choco uninstall audiowinfix -y
```
...
2. Fill in the three manifests in `packaging/winget/`, replacing every
   ...
   - `<FILL:URL>` the release asset URL of `AudioWinFix-win-Setup.exe`
   - `<FILL:SHA256>` its SHA-256, uppercase:
     `(Get-FileHash .\AudioWinFix-win-Setup.exe -Algorithm SHA256).Hash`
...
4. Copy them into your fork under
   `manifests/k/Kenshin9977/AudioWinFix/<version>/` and open a pull request.
...
`Kenshin9977.AudioWinFix` is permanent once merged. `Moniker: audiowinfix` is
