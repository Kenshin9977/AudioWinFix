...
| Requirement | Status |
| --- | --- |
| `.msi` or `.exe` installer | `AudioWinFix-win-Setup.exe`, from vpk |
...
| --- | --- |
| App type | EXE |
| Package URL | `https://github.com/Kenshin9977/AudioWinFix/releases/download/v<version>/AudioWinFix-win-Setup.exe` |
| Architecture | x64 |
...
AudioWinFix updates itself through Velopack. A Store install would then move to a
