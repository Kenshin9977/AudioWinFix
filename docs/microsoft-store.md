# Publishing AudioWinFix to the Microsoft Store
...
| Requirement | Status |
| --- | --- |
| `.msi` or `.exe` installer | `AudioWinFix-store-Setup.exe`, from vpk |
...
| --- | --- |
| App type | EXE |
| Package URL | `https://github.com/Kenshin9977/AudioWinFix/releases/download/v<version>/AudioWinFix-store-Setup.exe` |
| Architecture | x64 |
...
| | Artifact | Updates |
| --- | --- | --- |
| `win` | `AudioWinFix-win-Setup.exe` | the app updates itself from GitHub |
| `store` | `AudioWinFix-store-Setup.exe` | none; the Store is the update channel |
