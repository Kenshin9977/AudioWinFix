...
# A tray app, not a console tool. Left unshimmed, `audiowinfix` on the command
...
$exe = Join-Path $toolsDir 'AudioWinFix.exe'

if (Test-Path $exe) {
  Install-ChocolateyShortcut `
    -ShortcutFilePath (Join-Path ([Environment]::GetFolderPath('CommonPrograms')) 'AudioWinFix.lnk') `
    -TargetPath $exe `
    -Description 'Keeps your chosen audio devices selected'
}
